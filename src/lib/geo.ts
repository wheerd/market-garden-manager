...
export async function getSatelliteImageAsDataUri(
  ...
  const response = await fetch(
    `https://api.mapbox.com/styles/v1/mapbox/satellite-v9/static/${position.longitude.toFixed(
      6
    )},${position.latitude.toFixed(6)},${zoom.toFixed(1)},0},0/${width.toFixed(
      0
    )}x${height.toFixed(0)}?access_token=${MAPBOX_ACCESS_TOKEN}`
  );
  return await convertBlobToBase64(await response.blob());
}

export const metersPerPixel = (position: GeoPosition, zoom: number) =>
  (40007000 * Math.cos((position.latitude * Math.PI) / 180)) /
  (512 * Math.pow(2, zoom));
...
export async function getTimeZone(location: GeoPosition) {
  const response = await fetch(
    `https://secure.geonames.org/timezoneJSON?lat=${location.latitude}&lng=${location.longitude}&username=wheerd`
  );
